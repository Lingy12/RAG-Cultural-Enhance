...
for dataset in datasets:
    for i in range(1, num_prompt + 1):
        print('Loading vector store...')
        vs = EmbeddingStore(top_k=top_k, saved_vs=vector_store, model_name=model_name) 
        print('Vector store loaded.')
        ...
        eval_rag(rag=rag_model, dataset_name=dataset, prompt_index=i, eval_lang=['English'], eval_mode='zero_shot', model_name=str(Path(os.path.basename(vector_store)).with_suffix('')) + '_' + str(retrival_threshold) + '_' + str(top_k) + '_emb')
        # vs.close_cache()
