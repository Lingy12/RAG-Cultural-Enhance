...
def evaluate(model_path, prompt, vector_store, retrieval_threshold, top_k, eval_data, eval_mode, embed_name=None, verbose=1):
    ...
    prompt_n = Path(os.path.basename(model_path)).with_suffix('')
    ...
    # end_time = time.time()
    run_tag = '-'.join([str(model_path_n), str(prompt_n), str(vector_store_n), str(retrieval_threshold), str(top_k), str(embed_name)]) 
